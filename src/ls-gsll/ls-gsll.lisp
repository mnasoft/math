...
(defpackage :math/ls-gsll
  ...
  (:export solve
           solve-x)
  (:documentation
    "@b(Описание:) пакет @b(math/ls-gsll) определяет функции для
решения СЛАУ методом LU-разложения с использованием библиотеки GSLL."))
...
(defun solve-x (m-v)
  "@b(Описание:) функция @b(solve-x) возвращает корни решения СЛАУ
(системы линейных алгебраических уравнений), используя LU-разложение
матрицы @b(m-v).
