...
(defun make-vector-n (element n)
    "@b(Описание:) функция @b(make-vector-n) возвращает вектор длины @b(n),
все элементы которого равны @b(element).
...
(defun max-col-not-nil-value (lst)
  "@b(Описание:) функция @b(max-col-not-nil-value) возвращает список
максимальных значений по каждому столбцу матрицы (списка списков).

 @b(Пример использования:)
@begin[lang=lisp](code)
 (max-col-not-nil-value '((1 2 3) (4 1 5) (2 8 0))) => (4 8 5)
@end(code)"  
...
вектора @b(v). Норма результата равна 1.0.
...
(defmethod transform ((point-3d cons) (matrix-4x4 <matrix>))
  "@b(Описание:) метод @b(transform) применяет матрицу 4x4
к точке @b(point-3d) (3D) и возвращает новую точку в 3D.

 @b(Пример использования:)
@begin[lang=lisp](code)
 (transform '(1.0 2.0 3.0)
            (move-xyz 1.0 0.0 -1.0)) => (2.0 2.0 2.0)
@end(code)"
